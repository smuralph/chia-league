...
export function Live2026SeasonButton({ className = "" }: Live2026SeasonButtonProps) {
  ...
  const tooltipText = "Go to current season";

  const baseClassName = [
    "inline-flex items-center justify-center rounded-full border px-4 py-2 text-sm font-medium transition-colors",
    "border-zinc-500 bg-zinc-200 text-red-600 shadow-sm",
    className,
  ].join(" ");

  return (
    <div className="live-2026-season-button group relative inline-block" title={tooltipText}>
      <Link
        href="/seasons/2026"
        className={[baseClassName, "hover:bg-zinc-300"].join(" ")}
